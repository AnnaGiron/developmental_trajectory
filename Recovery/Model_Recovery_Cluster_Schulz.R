...
simMod= as.integer(commandArgs(TRUE)[2]) 
Rep= as.integer(commandArgs(TRUE)[3]) 
...
EnvFile=read.csv("../data/kwgdata_Schulz_env.csv")
...
fittedModel = fittedModel %>%
  ...
  mutate(experiment=factor(experiment, levels=c('Meder (2021)', 'Schulz (2019)', 'Adolescents')))%>%filter(experiment=='Schulz (2019)')
...
saveRDS(d2,paste0("./modelRecovery/Schulz_Sub_",uid,"_",newModels[simMod],"_",Rep,"_data.rds"))
saveRDS(allrecov,paste0("./modelRecovery/Schulz_Sub_",uid,"_",newModels[simMod],"_",Rep,"_data.rds"))
