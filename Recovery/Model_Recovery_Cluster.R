...
uid = 100 #as.integer(commandArgs(TRUE)[1]) 
simMod=3#as.integer(commandArgs(TRUE)[2]) 
...
acqlist<-list(greedyMean, epsilonGreedy, ucb) 
...
fittedModel = fittedModel %>%
  ...
  mutate(experiment=factor(experiment, levels=c('Meder (2021)', 'Schulz (2019)', 'Adolescent')))%>%filter(experiment=='Adolescent')
...
simP=c(fittedModel%>%filter(id==uid)%>%pull(lambda)%>%log(),
       fittedModel%>%filter(id==uid)%>%pull(beta)%>%log(),
       fittedModel%>%filter(id==uid)%>%pull(tau)%>%log(),
       fittedModel%>%filter(id==uid)%>%pull(epsilon)%>%logitTransform())# other transformation
...
source('./fit_parrallel_cluster.R')
#sim<-modelSimulateChoice(par=simP,rounds=1:10, k=unlist(newModelList[[simMod]][1]), acquisition=unlist(newModelList[[simMod]][2]),env=env,envSelector=environmentIndex)

sim<-modelSimulateChoice(par=simP,rounds=1:10, k=unlist(newModelList[[simMod]][1])[[1]], acquisition=unlist(newModelList[[simMod]][2])[[1]],env=env,envSelector=environmentIndex)
...
saveRDS(d2,paste0("./modelRecovery/Sub_",uid,"_",newModels[simMod],"_",Rep,"_data.rds"))
saveRDS(allrecov,paste0("./modelRecovery/Sub_",uid,"_",newModels[simMod],Rep,".rds"))


allrecov<-readRDS(paste0("./modelRecovery/Sub_",uid,"_",newModels[simMod],Rep,".rds"))


allrecov%>%mutate(kernel=unlist(kernel),aq=unlist(aq))%>%
  ggplot(aes(x=V2,fill=interaction(kernel,aq)))+
  geom_histogram()

allrecov%>%group_by(unlist(kernel),unlist(aq))%>%
  dplyr::summarize(n=n())
# 
# 
